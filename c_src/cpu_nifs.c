...
static int load(ErlNifEnv *env, void **priv_data, ERL_NIF_TERM load_info) {
  KERNEL_TYPE = enif_open_resource_type(env, NULL, "kernel", NULL, ERL_NIF_RT_CREATE, NULL);
  ARRAY_TYPE = enif_open_resource_type(env, NULL, "array_ref", dev_array_destructor, ERL_NIF_RT_CREATE, NULL);
  PINNED_ARRAY = enif_open_resource_type(env, NULL, "pinned_array", dev_pinned_array_destructor, ERL_NIF_RT_CREATE, NULL);
  return 0;
}
...
static ERL_NIF_TERM new_pinned_nif(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    ...
    host_matrix = (float*) malloc(data_size);
    if (host_matrix == NULL) {
...
static ERL_NIF_TERM new_gmatrex_pinned_nif(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    ...
    dev_matrix = (float*) malloc(data_size);

    memcpy(dev_matrix, matrix, data_size);
    if (dev_matrix == NULL) {
     return enif_make_badarg(env);
    }
...
static ERL_NIF_TERM new_ref_nif(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    ...
    //*((uint32_t*)dev_matrix) = 1; // rows
    //*(((uint32_t*)dev_matrix) + 1) = data_size; // cols
...
static ERL_NIF_TERM get_matrex_nif(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    ...
    memcpy(ptr_matrix, dev_array, data_size);
